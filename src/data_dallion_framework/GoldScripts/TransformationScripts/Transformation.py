                ...
                for dqm_log in unprocessed_transformation_files:
                    try:
                        ...
                        for source_detail in source_details[1:]:
                            if source_detail['transformation_type'] == "UNION":
                                ...
                                for col_, agg_func in measures.items():
                                    ...
                                    agg_exprs.append(func(F.col(col_)).alias(f"{col_}_{agg_func.lower()}"))
                        ...
                        final_result_df = final_result_df\
                            ...
                            .withColumn("eff_end_dt", to_date(lit("9999-12-31"), "yyyy-MM-dd"))\
                        ...
                        if DeltaTable.isDeltaTable(sparkSession=spark, identifier=target_table_location):
                            ...
                            target_table.alias("TARGET").merge(final_result_df.alias("staging"), f"TARGET.eff_end_dt = '9999-12-31' AND {primary_key_conditions}")\
                            ...
                            final_result_df.write.format("delta").mode("append")\
                                .partitionBy(*[c.strip() for c in target_table_details.transformation_partition_columns.split(",")])\
                                .save(target_table_location)
