...
class DataQualityCheck:
    def __init__(
        ...
        with OrchestrationProcess.OrchestrationProcess() as orch:
            self.dqm_unprocessed_files = orch.get_dqm_unprocessed_files(process_id, dataset_id)
    ...
    def start_dqm_check(self):
        for log in self.dqm_unprocessed_files:
            ...
            df = self.spark.read.format("delta").load(self.data_standardisation_location).filter(col("batch_id") == batch_id)
            original_df = df
    ...
    def _write_data(self, df:DataFrame, batch_id):
        ...
        df.write.format("delta").mode("append").partitionBy(self.staging_partition_columns).save(self.staging_location)
        df.write.format("delta").mode("append").partitionBy(self.publish_partition_columns).save(self.publish_location)
